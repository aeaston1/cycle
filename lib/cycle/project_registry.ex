defmodule Cycle.ProjectRegistry do
  ...
  @statuses ["active", "disabled", "error", "discovering"]
  ...
  defp validate_project(project, path) do
    ...
    |> Kernel.++(validate_repo(project["repo"], "#{path}.repo"))
    |> Kernel.++(validate_workflow(project["workflow"], "#{path}.workflow"))
  end
  ...
  defp validate_repo(nil, path), do: [Schema.error(path, "is required")]

  defp validate_repo(map, path) when is_map(map) do
    Enum.flat_map(["url", "full_name"], &Schema.required_string(map, &1, path))
  end

  defp validate_repo(_value, _path), do: []

  defp validate_workflow(nil, path), do: [Schema.error(path, "is required")]

  defp validate_workflow(map, path) when is_map(map) do
    Enum.flat_map(["path", "resolved_path"], &Schema.required_string(map, &1, path))
  end

  defp validate_workflow(_value, _path), do: []
